...
class GtfsHandler():
	...
	def get_blank_calendar_object(self, start_and_end):
		start = self.get_date_from_gtfs_date(start_and_end['start_date'])
		end = self.get_date_from_gtfs_date(start_and_end['end_date'])

		calendarObject = {}
		for d in self.daterange(start, end):
			d = self.get_gtfs_date_from_date(d)
			calendarObject[d] = None

		return calendarObject

	def get_date_from_gtfs_date(self, gtfs_date):
		return datetime.datetime.strptime(gtfs_date, "%Y%m%d")

	def get_gtfs_date_from_date(self, d):
	...
	def get_calendars_for_feed(self, gtfs_feed):
		...
		for d in calendars.keys():
			d = self.get_date_from_gtfs_date(d)
